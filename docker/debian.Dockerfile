...
RUN set -xe && \
    ...
    apt-get install -y $(echo $(wget --no-check-certificate -qO- https://raw.githubusercontent.com/9606/nas-tools/self/package_list_debian.txt)) && \
    ...
    pip install cython && \
    pip install -r https://raw.githubusercontent.com/9606/nas-tools/self/requirements.txt && \
    # Clear
...
ENV S6_SERVICES_GRACETIME=30000 \
    ...
    REPO_URL="https://github.com/9606/nas-tools.git" \
...
RUN set -xe \
    ...
    && git clone -b self ${REPO_URL} ${WORKDIR} --depth=1 --recurse-submodule \
